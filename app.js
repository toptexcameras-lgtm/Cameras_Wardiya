...
// =============================
// Week rollover rule
// Friday is OFF day, but we switch to the next week at Friday 18:00 (6 PM) local time.
// =============================
function getEffectiveNow(){
  const now = new Date();
  // JS getDay(): Sun=0..Sat=6, Friday=5
  if(now.getDay() === 5 && now.getHours() >= 18){
    // treat as next day (Saturday) so current week advances
    ...
    function renderDashboard(schedule, currentWeekIndex){
      ...
      cards.forEach((c)=>{
        ...
        card.innerHTML = `
          ...
          <span class="sr-only">Off</span></span>
            ${(() => {
              const offDate = new Date(week.weekEnd); // Observed holiday day (Thu)
              const offStr = formatDate(offDate);
              const holidaySet = getHolidaySet();
              if(holidaySet.has(offStr)){
                return `<div class="off-note holiday">متوقع إجازة (مُرحّلة للخميس) رسمية (مُرحّلة للخميس)</div>`;
              }
              return ``;
            })()}
          </div>
    ...
    function renderScheduleTable(scheduleToRender, currentWeekIndex){
      ...
      scheduleToRender.forEach((week)=>{
        ...
        row.innerHTML = `
          ...
          <td><span class="off-badge"><span aria-hidden="true"></span><span class="sr-only">Off</span></span>${(()=>{const offStr=formatDate(new Date(week.weekEnd)); return getHolidaySet().has(offStr) ? `<div class="off-note holiday">متوقع إجازة (مُرحّلة للخميس)</div>` : ``;})()}</td>
        `;
